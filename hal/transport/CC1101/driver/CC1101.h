 ...
 * | | CC1101 | INIT | IRQPIN=%%d                             | DIO1 pin number (if configured)
 * | | CC1101 | INIT | DIO3TCXO,VCONF:%%dX,DELAY:%%dms        | DIO 3 was configured as TCXO control with voltage (VCNONF) and start up delay (DELAY)ms
 * | | CC1101 | INIT | DIO2AntSw                              | DIO 2 was configured as antenna switch control
 * |!| CC1101 | INIT | SANCHK FAIL                            | Sanity check failed, check wiring or replace module
 * |!| CC1101 | INIT | ERR:0x%%X%%X                           | An error occured during init. See GetDeviceErrors in datasheet
 * | | CC1101 | RCV  | SEND ACK                               | ACK request received, sending ACK back
 * | | CC1101 | PTC  | LEVEL=%%d                              | Set TX power level
 * | | CC1101 | SAC  | SEND ACK,TO=%%d,RSSI=%%d,LQI=%%d       | Send ACK to node (TO), RSSI of received message (RSSI), LQI of message (LQI)
 * | | CC1101 | ATC  | ADJ TXL,cR=%%d,tR=%%d..%%d,rTXL=%%d    | Adjust TX level, current RSSI (cR), target RSSI range (tR), requested TX level (rTXL)
 * | | CC1101 | SWR  | SEND,TO=%%d,RETRY=%%d                  | Send message to (TO), NACK retry counter (RETRY)
 * | | CC1101 | SWR  | ACK FROM=%%d,SEQ=%%d,RSSI=%%d,LQI=%%d  | ACK received from node (FROM), seq ID (SEQ), (RSSI), (LQI)
 * |!| CC1101 | SWR  | NACK                                   | No ACK received
 * |!| CC1101 | CAD  |                                        | Could not send, channel active
 * | | CC1101 | SPP  | PCT=%%d,TX LEVEL=%%d                   | Set TX level percent (PCT), TX level (LEVEL)
 * | | CC1101 | PWD  |                                        | Power down radio
 * | | CC1101 | PWU  |                                        | Power up radio
 *
 * CC1101 modem configuration
 *
 * BW = Bandwidth in kHz
...
#define CC1101_MAX_PACKET_LEN (0x40u) //!< This is the maximum number of bytes that can be carried by the modem.
#define CC1101_PREAMBLE_LENGTH (8u)   //!< Preamble length, default=8
#define CC1101_BROADCAST_ADDRESS 0xFF
#define CC1101_ATC_TARGET_RANGE_DBM (2u) //!< ATC target range +/- dBm
...
#define MY_CC1101_TX_TIMEOUT_MS (50ul) //!< Timeout before TX is considered failed.
#endif

#if !defined(MY_CC1101_TX_RETRY_DELAY_MS)
#define MY_CC1101_TX_RETRY_DELAY_MS (50l) //!< Delay before reattempting transmit.
#endif
...
#define MY_CC1101_ACK_SEND_DELAY_MS 5 //!< Delay before sending ACK packet, to give slow nodes a chance to flip to RX.
#endif
...
#if (MY_CC1101_MAX_POWER_LEVEL_DBM) <= (MY_CC1101_MIN_POWER_LEVEL_DBM)
#error CC1101_MAX_POWER_DBM has to be bigger than CC1101_MIN_POWER_DBM
#endif

#if !defined(CC1101_RETRY_TIMEOUT_MS)
// air-time approximation for timeout, 1 hop ~15 bytes payload - adjust if needed
// <fill in>
#define CC1101_RETRY_TIMEOUT_MS (1000ul) //!< Timeout for ACK, adjustments needed if modem configuration changed (air time different)
#endif
 ...
 */
typedef uint16_t cc1101_sequenceNumber_t; // will eventually change to uint8_t in 3.0
/**
...
typedef struct
{
    ...
    cc1101_sequenceNumber_t sequenceNumber; //!< Packet sequence number, used for ACK
...
#define CC1101_HEADER_LEN sizeof(cc1101_header_t)                          //!< Size header inside payload
#define CC1101_MAX_PAYLOAD_LEN (CC1101_MAX_PACKET_LEN - CC1101_HEADER_LEN) //!< Max payload length

#define CC1101_PACKET_HEADER_VERSION (1u)     //!< CC1101 packet header version
#define CC1101_MIN_PACKET_HEADER_VERSION (1u) //!< Minimal CC1101 packet header version
...
typedef struct
{
    union
    {
        struct
        {
            ...
            union
            {
                uint8_t payload[CC1101_MAX_PAYLOAD_LEN]; //!< Payload, i.e. MySensors message
 ...
 **/
static void CC1101_readRegisterBurst(cc1101_registers_t reg, uint8_t *buffer, uint8_t size);
 ...
 **/
static void CC1101_sendRegisterBurst(cc1101_registers_t reg, const uint8_t *buffer, uint8_t size);
 ...
 */
static bool CC1101_send(const uint8_t recipient, uint8_t *data, const uint8_t len,
                        const cc1101_controlFlags_t flags);
/**
 ...
 */
static void CC1101_sendAck(const uint8_t recipient, const cc1101_sequenceNumber_t sequenceNumber,
