...
static bool CC1101_initialise()
{
	...
	CC1101.powerLevel = MY_CC1101_POWER_2;
	CC1101.targetRSSI = MY_CC1101_ATC_TARGET_DBM;
...
static void CC1101_handle()
{
	if (CC1101.irqFired)
	{
		...
		CC1101_DEBUG_V(PSTR("CC1101:IRQ:RXLEN=%d(%d):TXLEN=%d(%d):CHST=0x%02x\n"),
					 rxBytesLen, rxBytesReg >> 7, txBytesLen, txBytesReg >> 7, CC1101.chipStatus);
		// Transmission done
		...
		if (rxBytesLen)
		{
			if (!(rxBytesReg >> 7))
			{
				...
				CC1101.currentPacket.LQI = CC1101.currentPacket.data[rxBytesLen - 1] & 0x7F;
				...
				if (CC1101.currentPacket.header.version >= CC1101_MIN_PACKET_HEADER_VERSION)
				{
					CC1101.ackReceived = CC1101.currentPacket.header.controlFlags.fields.ackReceived &&
										 !CC1101.currentPacket.header.controlFlags.fields.ackRequested;
					CC1101.dataReceived = !CC1101.ackReceived;
					CC1101.currentPacket.header.length++; // TX len is short by one for radio.
				}
			}
			else
			{
...
static bool CC1101_csnLow()
{
	...
	for (unsigned long start = hwMillis(); hwMillis() - start < MY_CC1101_CSN_TIMEOUT_MS;)
	{
...
static void CC1101_wakeUp()
{
	...
	CC1101_csnHigh();
	CC1101.radioMode = CC1101_MODE_IDLE;
	CC1101_txPower(CC1101.powerLevel); // PATABLE cleared on sleep. [Datasheet section 24]
	CC1101_rx();
}
								 ...
								 const uint8_t bufferSize, const bool noACK)
{
	...
	{
		CC1101_DEBUG(PSTR("CC1101:SWR:SEND,TO=%u,LEN=%d,SEQ=%u,RETRY=%u,NOACK=%d\n"),
					 recipient,
					 bufferSize,
					 CC1101.txSequenceNumber,
					 retry, noACK);
		...
		while (hwMillis() < start + MY_CC1101_SEND_TIMEOUT_MS)
		{
			...
			if (CC1101.ackReceived)
			{
				...
				if (CC1101.currentPacket.header.sender == recipient &&
					CC1101.currentPacket.ACK.sequenceNumber == CC1101.txSequenceNumber)
				{
    ...
    /*
		if (CC1101.ATCenabled)
		{
			CC1101_txPower(CC1101.powerLevel + 1); // increase power, maybe we are far away from gateway
		}
    */
	}
	return false;
...
{
	if (CC1101.radioMode != CC1101_MODE_RX && CC1101.radioMode != CC1101_MODE_TX)
	{
...
static uint8_t CC1101_getData(uint8_t *buffer, const uint8_t bufferSize)
{
	...
	if (buffer != NULL)
	{
		(void)memcpy((void *)buffer, (void *)&CC1101.currentPacket.payload, payloadSize);
	}
	// clear data flag
	CC1101.dataReceived = false;
	// ACK handling
...
static void CC1101_setFrequency()
{
	...
	CC1101_sendRegisterBurst(CC1101_REG_FREQ2, freq, sizeof(freq) / sizeof(uint8_t));
...
{
	const cc1101_powerLevel_t newPowerLevel = static_cast<cc1101_powerLevel_t>(MY_CC1101_MIN_POWER_LEVEL_DBM + (MY_CC1101_MAX_POWER_LEVEL_DBM - MY_CC1101_MIN_POWER_LEVEL_DBM) * (newPowerPercent / 100.0f));
